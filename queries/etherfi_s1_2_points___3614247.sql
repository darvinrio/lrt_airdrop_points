...
-- query name: etherfi_s1_2_points
-- query link: https://dune.com/queries/3614247
...
aggregated as (
    select
        date_trunc('week', wallet_start_time) as date,
        token,
        symbol,
        round(avg(
            case when action_rank=1 then amt_cumulative else null end
        ),3) as balance,
        sum(amt_cumulative*earning_seconds*multiplier*eff_boost*10000/(24*60*60)) as pearl_points,
        sum(amt*mint_flag*40) as mint_pearl_points,        
        sum(
            sum(amt_cumulative*earning_seconds*multiplier*eff_boost*10000/(24*60*60))
        ) over(
            partition by token
            order by date_trunc('week', wallet_start_time)
        ) as token_pearl_points,
        sum(
            sum(amt_cumulative*earning_seconds*multiplier*eff_boost*10000/(24*60*60))
        ) over(order by date_trunc('week', wallet_start_time)) as total_pearl_points,
        sum(
            sum(amt*mint_flag*40)
        ) over(order by date_trunc('week', wallet_start_time)) as total_mint_pearl_points
...
-- --     prev_time_in_system,
-- --     eff_boost,
-- --     eff_boost *boost/(earning_seconds) as eff_boost_2,
--     amt_cumulative*earning_seconds*multiplier*eff_boost*10000/(24*60*60) as point
...
-- from stake_rank_join 
-- -- -- -- -- from math
-- -- order by evt_block_time, evt_index, wallet_end_time
-- order by point desc
-- -- order by wallet_start_time, wallet_end_time, start_hour
-- limit 1000
-- 0xc93297d8faa629acd516578517364f749cb387ad
