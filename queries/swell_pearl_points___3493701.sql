...
-- query name: swell_pearl_points
-- query link: https://dune.com/queries/3493701
...
kelp_multipliers as (
    select * from ( values
        ...
        (0x4afdb1b0f9a56922e398d29239453e6a06148ed0, 4.50, date'2023-04-12', current_date), -- 'rsweth pendle yt'
...
aggregated as (
    select 
        ...
        sum(amt_cumulative*earning_seconds*multiplier*4/(24*60*60)) as pearl_points,
        sum(amt*mint_flag*40) as mint_pearl_points,        
        sum(
            sum(amt_cumulative*earning_seconds*multiplier*4/(24*60*60))
        ) over(
            partition by token
        ...
        sum(
            sum(amt_cumulative*earning_seconds*multiplier*4/(24*60*60))
        ) over(order by date_trunc('week', evt_block_time)) as total_pearl_points,
        sum(
            sum(amt*mint_flag*40)
        ) over(order by date_trunc('week', evt_block_time)) as total_mint_pearl_points
    from math 
    group by 1,2,3
)
