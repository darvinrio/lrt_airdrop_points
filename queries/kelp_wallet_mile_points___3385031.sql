...
-- query name: kelp_wallet_mile_points
-- query link: https://dune.com/queries/3385031
...
aggregated as (
    select 
        ...
        round(avg(
            case when symbol = 'rsETH' and action_rank=1 then boosted_amt_cumulative+amt_cumulative else null end
        ),3) as rsETH_balance,
        round(avg(
            case when symbol = 'univ3' and action_rank=1 then amt_cumulative else null end
        ),3) as rsETH_ETHx_univ3_balance,
        round(avg(
            case when symbol = 'crv' and action_rank=1 then amt_cumulative else null end
        ),3) as rsETHsfrx_crv_balance,
        round(avg(
            case when symbol = 'bal' and action_rank=1 then amt_cumulative else null end
        ),3) as rsETH_ETHx_bal_balance,
        round(avg(
            case when symbol = 'mav' and action_rank=1 then amt_cumulative else null end
        ),3) as mav_balance,
        round(avg(
            case when symbol = 'pendleLP' and action_rank=1 then amt_cumulative else null end
        ),3) as pendleLP_balance,
        round(avg(
            case when symbol = 'YT_rsETH' and action_rank=1 then amt_cumulative else null end
        ),3) as YT_rsETH_balance,
        ...
        sum(
            case when symbol = 'rsETH' then
                ...
                -- then amt_cumulative*earning_seconds*multiplier*10000/(24*60*60) 
            else 0 end
        ) as rsETH_kelp_miles,
        sum(
            case when symbol = 'univ3' then amt_cumulative*earning_seconds*multiplier*10000/(24*60*60) else 0 end
        ) as rsETH_ETHx_univ3_kelp_miles,
        sum(
            case when symbol = 'crv' then amt_cumulative*earning_seconds*multiplier*10000/(24*60*60) else 0 end
        ) as rsETHsfrx_crv_kelp_miles,
        sum(
            case when symbol = 'bal' then amt_cumulative*earning_seconds*multiplier*10000/(24*60*60) else 0 end
        ) as rsETH_ETHx_bal_kelp_miles,
        sum(
            case when symbol = 'mav' then amt_cumulative*earning_seconds*multiplier*10000/(24*60*60) else 0 end
        ) as mav_kelp_miles,
        sum(
            case when symbol = 'pendleLP' then amt_cumulative*earning_seconds*multiplier*10000/(24*60*60) else 0 end
        ) as pendleLP_kelp_miles,
        sum(
            case when symbol = 'YT_rsETH' then amt_cumulative*earning_seconds*multiplier*10000/(24*60*60) else 0 end
        ) as YT_rsETH_kelp_miles,
        ...
        -- sum(amt_cumulative*earning_seconds*multiplier*10000/(24*60*60)) as total_kelp_miles,
        -- sum(
        --     case when symbol = 'rsETH' then amt_cumulative*earning_seconds/(60*60) else 0 end
        -- ) as eigen_miles,
...
select
    rank() over(order by total_kelp_miles desc) as miles_rank,
    *
    -- rsETH_kelp_miles+
    --     rsETH_ETHx_univ3_kelp_miles+
    --     rsETHsfrx_crv_kelp_miles+
    --     rsETH_ETHx_bal_kelp_miles
    -- as total_kelp_miles
from aggregated 
order by total_kelp_miles desc
